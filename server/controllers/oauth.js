...
exports.createCustomer = async (req, res) => {
    try {
        ...
        let url = `http://localhost:5000/oauth/confirm/${encodeURI(tokenEmail)}`;
...
exports.confirmCustomer = async (req, res) => {
    try {
        ...
        let idClient = jwt.verify(emailtoken, config.JWT_EMAIL_SECRET);
...
exports.createCustomerSocialNetwork = async (req, res) => {
    ...
    try {
        ...
        await jwt.sign({data: Customer}, config.JWT_SECRET, {
            expiresIn: 36000
        }, function (err, token) {
